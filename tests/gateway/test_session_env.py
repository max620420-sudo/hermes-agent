...
def test_set_session_env_binds_durable_session_id():
    """Gateway tool execution must receive the same durable session id as the agent."""
    runner = object.__new__(GatewayRunner)
    source = SessionSource(
        platform=Platform.TELEGRAM,
        chat_id="1001",
    ...
    tokens = runner._set_session_env(context)
    try:
        assert get_session_env("HERMES_SESSION_ID") == "telegram-session-1"
    finally:
        runner._clear_session_env(tokens)
